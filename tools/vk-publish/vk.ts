...
/**
 * Ошибки, после которых продолжать очередь бессмысленно: следующие сотни
 * карточек упрутся в то же самое. 214 — исчерпан суточный лимит записей
 * (у сообщества это порядка 50 постов), 5/27 — протух или сменился токен,
 * 15 — закрыт доступ к стене.
 */
export const FATAL_ERROR_CODES = new Set([5, 15, 27, 214]);
